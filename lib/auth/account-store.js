...
async function getUserAccountOverview({ queryDb, userId }) {
  const result = await queryDb(
    `
      SELECT
        (SELECT COUNT(*)::int FROM user_saved_titles WHERE user_id = $1 AND COALESCE((movie_payload->>'savedToWatchlist')::boolean, TRUE) = TRUE) AS saved_titles_count,
        (SELECT COUNT(*)::int FROM user_saved_people WHERE user_id = $1) AS saved_people_count,
        (SELECT COUNT(*)::int FROM user_saved_titles WHERE user_id = $1 AND COALESCE((movie_payload->>'watched')::boolean, FALSE) = TRUE) AS watched_titles_count,
        (SELECT COUNT(*)::int
         FROM user_sessions
         WHERE user_id = $1
