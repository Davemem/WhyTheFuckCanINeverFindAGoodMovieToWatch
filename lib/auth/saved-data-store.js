...
function normalizeSavedMoviePayload(movie) {
  const titleIdentity = movie && identity(movie);
  ...
  return {
    ...
    savedToWatchlist: movie.savedToWatchlist !== false,
  };
}
...
async function listUserSavedTitles({ queryDb, userId }) {
  const result = await queryDb(
    `
      SELECT movie_id, media_type, movie_payload, created_at, updated_at
  ...
  return result.rows
    .map((row) => {
      ...
      return {
        ...payload,
        ...identity({ id: row.movie_id, mediaType: row.media_type || "movie" }),
        savedAt: payload.savedAt || row.created_at,
        updatedAt: row.updated_at,
      };
    })
    .filter((movie) => identity(movie));
}
...
async function saveUserTitle({ queryDb, userId, movie }) {
  ...
  await queryDb(
    `
      ...
      VALUES ($1, $2, $4, $3::jsonb)
      ON CONFLICT (user_id, media_type, movie_id) DO UPDATE
      SET
        movie_payload = user_saved_titles.movie_payload || (EXCLUDED.movie_payload - 'watched'),
        updated_at = NOW()
    `,
    [userId, normalizedMovie.tmdbId, JSON.stringify({ ...normalizedMovie, savedToWatchlist: true }), normalizedMovie.mediaType],
  );
...
async function removeUserTitle({ queryDb, userId, movieId }) {
  const titleIdentity = identity(movieId);
  if (!titleIdentity) {
  ...
  await queryDb(
    `
      ...
      WHERE user_id = $1
        AND movie_id = $2
        AND media_type = $3
        AND COALESCE(movie_payload->'watched' = 'true'::jsonb, FALSE) = TRUE
    `,
    [userId, titleIdentity.tmdbId, titleIdentity.mediaType],
  );
  ...
  await queryDb(
    `
      ...
      WHERE user_id = $1
        AND movie_id = $2
        AND media_type = $3
        AND COALESCE(movie_payload->'watched' = 'true'::jsonb, FALSE) = FALSE
    `,
    [userId, titleIdentity.tmdbId, titleIdentity.mediaType],
  );
...
async function saveUserWatchedTitle({ queryDb, userId, movie }) {
  ...
  await queryDb(
    `
      INSERT INTO user_saved_titles (user_id, movie_id, media_type, movie_payload)
      VALUES ($1, $2, $4, $3::jsonb)
      ON CONFLICT (user_id, media_type, movie_id) DO UPDATE
      SET
        ...
        updated_at = NOW()
    `,
    [userId, normalizedMovie.tmdbId, JSON.stringify({ ...normalizedMovie, watched: true, savedToWatchlist: false }), normalizedMovie.mediaType],
  );
  ...
  await queryDb(
    `
      ...
      WHERE user_id = $1
        AND movie_id = $2
        AND media_type = $3
        AND COALESCE(movie_payload->'savedToWatchlist' <> 'false'::jsonb, TRUE) = TRUE
    `,
    [userId, titleIdentity.tmdbId, titleIdentity.mediaType],
  );
  await queryDb(
    `
      ...
      WHERE user_id = $1
        AND movie_id = $2
        AND media_type = $3
        AND COALESCE(movie_payload->'savedToWatchlist' <> 'false'::jsonb, TRUE) = FALSE
    `,
    [userId, titleIdentity.tmdbId, titleIdentity.mediaType],
  );
  return titleIdentity.id;
