...
The home page is a dark, search-first discovery library. One debounced search finds movies, TV shows, people and studios, with category shortcuts, immediate submission, cancellation of stale responses and partial-error recovery. Search results and selected-person credits appear above the collections without replacing them. Titles can be saved directly from compact cards or opened for details and streaming availability.

All pages share the charcoal-and-gold theme, navigation and responsive controls through `site-theme.css`. The watchlist reuses discovery's actual-score, award, genre, year, runtime and series-status filters, alongside search, watched status and rating sorts. Missing metadata loads in bounded batches, with explicit retry on failure and account-isolated in-memory details. Saved profiles have searchable horizontal categories for actors, writers, directors, producers and studios; ambiguous older filmmaker records remain accessible in their own category. Studios open company movie/TV catalogues. Account security and saved-data storage contracts are unchanged.

Seven permanent horizontal collections suggest up to 50 movies, TV shows, actors, writers, directors, producers and studios each. Another 20 collections cover individual genres. Each row has independent refresh, labelled left/right buttons, keyboard scrolling and touch/trackpad scrolling. Genre rows load as they approach the viewport. The daily selection is stable until that row is refreshed; limited demo/offline catalogues show their actual counts rather than duplicated filler.

Global filters trim each row's existing selection without silently refilling it: media type, genre, IMDb, Rotten Tomatoes, Metacritic, TMDb, minimum TMDb votes, year range, runtime, series status and watched status. Awards support winners, nominees or either, for any award, Oscars, Emmys, Golden Globes, BAFTAs, SAG and Critics Choice. Award matching uses OMDb summary clauses, not an exhaustive awards database. A missing external rating never falls back to a TMDb estimate. Verification is queued and cached, with checking counts and retry controls. People and studios qualify if any of up to two featured movies and two featured TV shows satisfies all filters; this is explicitly not a full-career award/ratings search. Title sorting supports source ratings, year and A–Z; people/studios support suggested order and A–Z.

`GET /api/suggestions?kind=movie|tv|actors|writers|directors|producers|studios|genre&genre=18&seed=0` returns an independently refreshable cohort (up to 50). `GET /api/suggestion-credits?kind=writers&id=...` returns role-specific featured credits for bounded filtering. TMDb movie and television genre IDs are mapped separately. People suggestions reuse the existing ranking pipeline; curated studios are resolved to TMDb companies for credits. Saved-person catalogues include movies and television. Older `/people.html` directory links redirect into the equivalent home-page state.
